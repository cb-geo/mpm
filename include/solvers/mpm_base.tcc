...
//! MPM state_vars smoothing
template <unsigned Tdim>
void mpm::MPMBase<Tdim>::state_vars_smoothing(const std::string& var,
                                              unsigned phase) {
  // Assign state_vars to nodes
  mesh_->iterate_over_particles(
      std::bind(&mpm::ParticleBase<Tdim>::map_state_vars_to_nodes,
                std::placeholders::_1, std::placeholders::_2, phase));
  ...
  // Smooth pressure over particles
  mesh_->iterate_over_particles(
      std::bind(&mpm::ParticleBase<Tdim>::compute_state_vars_smoothing,
                std::placeholders::_1, std::placeholders::_2, phase));
}