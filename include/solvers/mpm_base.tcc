    ...
    try {
      if (analysis_.find("newmark") != analysis_.end()) {
        if (!initialise_newmark(analysis_.at("newmark")))
          throw std::runtime_error("Parameters of Newmark scheme are not defined");
      }
    } catch (std::exception& exception) {
      console_->warn("{} #{}: Parameters of Newmark scheme is not specified, using "
...
bool mpm::MPMBase<Tdim>::initialise_newmark(const Json& newmark_props) {
  ...
  try {
    ...
    console_->warn("#{}: Parameters of Newmark scheme are undefined {} ", __LINE__,
                   exception.what());
    status = false;
  }
