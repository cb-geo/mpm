...
//! Write particles to HDF5
template <unsigned Tdim>
bool mpm::Mesh<Tdim>::read_particles_hdf5(
    const std::string& filename, const std::set<std::string>& particle_types) {
  // Vector of particles
  ...
  for (const std::string& particle_type : particle_types) {
    ...
    if (file_id < 0)
      throw std::runtime_error("HDF5 particle file is not found");
      ...
      if ((*citr)->status()) {
        ...
        if (!internal) {
          if ((*citr)->volume_fraction() < volume_tolerance) {
            ...
            for (const auto n_id : (*citr)->neighbours()) {
              if (map_cells_[n_id]->volume_fraction() < volume_tolerance) {
                cell_at_interface = true;
                const auto& n_node_id = map_cells_[n_id]->nodes_id();
