...
template <>
inline void
    mpm::Particle<1>::map_multimaterial_normal_traction_to_nodes() noexcept {
  // Compute this particle's traction normal to the contact and map it to the
  // nodes (stored in the nodal property pool
  for (unsigned i = 0; i < nodes_.size(); ++i) {
    Eigen::Matrix<double, 1, 1> normal =
        nodes_[i]->property("normal_unit_vectors", this->material_id(), 1);
...
void mpm::Particle<Tdim>::compute_strain(double dt, bool interface) noexcept {
  ...
  // Compute at centroid
  // Strain rate for reduced integration
  Eigen::Matrix<double, 6, 1> strain_rate_centroid = this->compute_strain_rate(
      dn_dx_centroid_, mpm::ParticlePhase::Solid, interface);
