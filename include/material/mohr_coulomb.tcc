...
Eigen::Matrix<double, 6, 1> mpm::MohrCoulomb<Tdim>::compute_stress(
  ...
  double yield = 0.;
  if (yield_type_trial == FailureState::Tensile) yield = yield_function(0);
  if (yield_type_trial == FailureState::Shear) yield = yield_function(1);
  ...
  // Compute plastic volume strain
  double plastic_vstrain = (*state_vars).at("plastic_strain0") +
