...
int mpm::MohrCoulomb<Tdim>::check_yield(Eigen::Matrix<double,2,1>& _yield_function, const double _epsilon, const double _rho, const double _theta){
  ...
  if(yield_tension > 1.E-22 && yield_shear > 1.E-22){
    ...
    double h = sqrt(2. / 3.) * cos(_theta) * _rho + _epsilon / sqrt(3.) - tension_cutoff_+
           alphaP * (sqrt(2. / 3.) * cos(_theta - 4. * PI /3.) * _rho + _epsilon / sqrt(3.)  - SigmaP);
    if(h > 1.E-22)_yield_type = 1;
    else _yield_type = 2;
  }
  else if(yield_shear > 1.E-22 && yield_tension < 1.E-22)_yield_type = 2;
  else if(yield_tension > 1.E-22 && yield_shear < 1.E-22)_yield_type = 1;
  return _yield_type;
}

//! Return dF/dSigma and dP/dSigma
template <unsigned Tdim>
bool mpm::MohrCoulomb<Tdim>::compute_df_dp(const int _yield_type,
                       const double _j2, const double _j3,
  ...
  if(_yield_type == 1){
    double et_value = 0.6;
    double xit =0.1;
    double Rt_den = 2. * (1 - et_value * et_value ) * cos(_theta) + (2. * et_value -1) *
                    sqrt(4. * (1 - et_value * et_value )  * cos(_theta) * cos(_theta) + 5.* et_value * et_value - 4. * et_value);
    double Rt_num = 4. * (1 - et_value * et_value ) * cos(_theta) * cos(_theta) + (2. * et_value -1) * (2. * et_value -1);
    double Rt = Rt_num / (3. * Rt_den);
    double dP_dRt = 1.5 * _rho * _rho * Rt / sqrt(xit * xit * tension_cutoff_ * tension_cutoff_ + 1.5 * Rt * Rt * _rho * _rho);
    double dP_dRho = 1.5 * _rho * Rt * Rt / sqrt(xit * xit * tension_cutoff_ * tension_cutoff_ + 1.5 * Rt * Rt * _rho * _rho);
    double dP_dEpsilon = 1. / sqrt(3.);
    double dRtden_dTheta = - 2. * (1 - et_value * et_value ) * sin(_theta) - (2. * et_value -1) * 4. * (1 - et_value * et_value ) * cos(_theta) * sin(_theta)
                    / sqrt(4. * (1 - et_value * et_value )  * cos(_theta) * cos(_theta) + 5.* et_value * et_value - 4. * et_value);
    double dRtnum_dTheta = -8. * (1 - et_value * et_value ) * cos(_theta) * sin(_theta);
    double dRt_dTheta = (dRtnum_dTheta * Rt_den - dRtden_dTheta * Rt_num) / (3. * Rt_den * Rt_den);
...
Eigen::Matrix<double, 6, 1> mpm::MohrCoulomb<Tdim>::compute_stress(
  ...
  const double epds = (2. / 3.) * sqrt(3. / 2. * (PDS(0) * PDS(0) + PDS(1) * PDS(1) + PDS(2) * PDS(2))
  ...
  if (yield_type !=1 && (epds - peak_EPDS_) > 0. && (crit_EPDS_ - epds) > 0.) {
    ...
    phi_ = phi_min; psi_ = psi_min; c_ = c_min;
  }
  ...
  if(yield_type_trial == 1)
    ...
    lambda_trial = yield_function_trial(1) / ((dF_dSigma_trial.transpose() * de_).dot(dP_dSigma_trial.transpose()) + softening_trial);
  ...
  Vector6d stress_update = trial_stress -  (p_multiplier * this->de_ * dP_dSigma_Final);
  ...
  if(PDS_update){
    ...
    dPDS(0) -= (1. /3. * dpvstrain); dPDS(1) -= (1. /3. * dpvstrain); dPDS(2) -= (1. /3. * dpvstrain);
    dPDS(3) = 0.5 * dpstrain(3); dPDS(4) = 0.5 * dpstrain(4); dPDS(5) = 0.5 * dpstrain(5);
    PDS += dPDS;
    ptr-> update_PDS(PDS);
  }
