...
class DiscontinuityBase {
 public:
  ...
  virtual bool create_elements(const std::vector<std::vector<mpm::Index>>& elements) {return true;};

  //return the levelset values of each doordinates
  //! \param[in] the vector of the coordinates
  virtual void compute_levelset(const std::vector<VectorDim>& coordinates, std::vector<double>& phi_list)  = 0;
