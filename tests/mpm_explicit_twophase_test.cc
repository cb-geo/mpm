...
// Check MPM Explicit
TEST_CASE("MPM 2D Explicit Two Phase implementation is checked",
          "[MPM][2D][Explicit][2Phase]") {
  // Dimension
  ...
  char* argv[] = {(char*)"./mpm",
                  (char*)"-f",  (char*)"./",
                  (char*)"-i",  (char*)"mpm-explicit-twophase-2d.json"};
  // clang-format on
...
// Check MPM Explicit
TEST_CASE("MPM 3D Explicit Two Phase implementation is checked",
          "[MPM][3D][Explicit][2Phase]") {
  // Dimension
