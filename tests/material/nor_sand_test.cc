...
//! Check NorSand class in 2D
TEST_CASE("NorSand is checked in 2D", "[material][linear_elastic][2D]") {
  // Tolerance
  ...
  SECTION("NorSand id is zero") {
    ...
    auto material =
        Factory<mpm::Material<Dim>, unsigned, const Json&>::instance()->create(
            "NorSand2D", std::move(id), jmaterial);
  ...
  SECTION("NorSand check thermodynamic pressure") {
    unsigned id = 0;
    auto material =
        Factory<mpm::Material<Dim>, unsigned, const Json&>::instance()->create(
            "NorSand2D", std::move(id), jmaterial);
...
TEST_CASE("NorSand is checked in 3D", "[material][linear_elastic][3D]") {
  // Tolerance
  const double Tolerance = 1.E-7;
