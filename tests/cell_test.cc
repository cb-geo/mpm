...
TEST_CASE("Cell is checked for 2D case", "[cell][2D]") {
  ...
  SECTION("Test particle information mapping") {
    ...
    SECTION("Check inverse rotation matrix") {
      ...
      double beta = 1 * M_PI / 180;
      ;  // Check for non-zero beta
      double gamma = 30 * M_PI / 180;
      ...
      auto check_inverse_rotation_matrix = cell->compute_inverse_rotation_matrix(alpha, beta, gamma);
      REQUIRE(check_inverse_rotation_matrix.cols() == 2);
      REQUIRE(check_inverse_rotation_matrix.rows() == 2);

      REQUIRE(check_inverse_rotation_matrix(0, 0) == Approx(inverse_rotation_matrix(0, 0)).epsilon(Tolerance));  
      REQUIRE(check_inverse_rotation_matrix(1, 0) == Approx(inverse_rotation_matrix(1, 0)).epsilon(Tolerance));  
      REQUIRE(check_inverse_rotation_matrix(0, 1) == Approx(inverse_rotation_matrix(0, 1)).epsilon(Tolerance));
      REQUIRE(check_inverse_rotation_matrix(1, 1) == Approx(inverse_rotation_matrix(1, 1)).epsilon(Tolerance));  
    }
  }
}
...
TEST_CASE("Cell is checked for 3D case", "[cell][3D]") {
  ...
  SECTION("Test particle information mapping") {
    ...
    SECTION("Check inverse rotation matrix") {
      ...
      REQUIRE(check_inverse_rotation_matrix(0, 0) == Approx(inverse_rotation_matrix(0, 0)).epsilon(Tolerance));  
      REQUIRE(check_inverse_rotation_matrix(0, 1) == Approx(inverse_rotation_matrix(0, 1)).epsilon(Tolerance));  
      REQUIRE(check_inverse_rotation_matrix(0, 2) == Approx(inverse_rotation_matrix(0, 2)).epsilon(Tolerance));
      REQUIRE(check_inverse_rotation_matrix(1, 0) == Approx(inverse_rotation_matrix(1, 0)).epsilon(Tolerance));  
      REQUIRE(check_inverse_rotation_matrix(1, 1) == Approx(inverse_rotation_matrix(1, 1)).epsilon(Tolerance));  
      REQUIRE(check_inverse_rotation_matrix(1, 2) == Approx(inverse_rotation_matrix(1, 2)).epsilon(Tolerance));
      REQUIRE(check_inverse_rotation_matrix(2, 0) == Approx(inverse_rotation_matrix(2, 0)).epsilon(Tolerance));  
      REQUIRE(check_inverse_rotation_matrix(2, 1) == Approx(inverse_rotation_matrix(2, 1)).epsilon(Tolerance));  
      REQUIRE(check_inverse_rotation_matrix(2, 2) == Approx(inverse_rotation_matrix(2, 2)).epsilon(Tolerance));
